...
~$#Space::
	Sleep, 200
	CoordMode, Pixel, Screen
	ImageSearch, , , 1618, 1042, 1707, 1079, *80 D:\Users\Bruno\Documents\Scripts\Shortcuts\Images\Japanese.png
	if(!ErrorLevel){
		Send, +{CapsLock}
	}
	CoordMode, Pixel, Relative
...
isOnYt:
	...
	ytLogo2 := ErrorLevel
	ImageSearch, , , 110, 130, 260, 180, *50 D:\Users\Bruno\Documents\Scripts\Shortcuts\Images\YTApps.png
	ytApps := ErrorLevel
Return
actionOnYt:
	if(!ytLogo || !ytLogo2 || !ytApps){
...
#IfWinActive, ahk_exe vivaldi.exe
	~^2::
		...
		while(GetKeyState("Control", "P")){
			ToolTip, Release Ctrl
			Process, Exist 
			hwnd := WinExist("ahk_class tooltips_class32 ahk_pid " ErrorLevel) 
			WinGetPos,,,w,h,ahk_id %hwnd% 
			While (DllCall("MoveWindow","Uptr",hwnd,"Int",x+20,"Int",y-10,"Int",w,"Int",h,"Int",0) && GetKeyState("Control", "P")){
				if(GetKeyState("1", "P")){
					KeyWait, 1
					doubleTapCounter++
	...
	~^1::
		...
		Gosub, isOnYt
		Gosub, actionOnYt
		while(GetKeyState("Control", "P") && doubleTap = 0){
			if(GetKeyState("1", "P")){
				doubleTap := 1
			}
